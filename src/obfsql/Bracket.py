...
class Bracket:
	"""
	This class creates and recovers all of the data for a specific tournament/event for Open Bracket Format.
	...
	def create_tables(self):
		...
		from obfmodels import TournamentObf, PersonalInformationObf, CharacterObf
		import traceback
		...
		try:
			with self.db:
				tables = [
					...
					PhaseObf,
					TournamentObf,
					PersonalInformationObf,
					CharacterObf,
				]
				for ig in tables:
					# print(str(ig))
					self.db.create_tables([ig])
			# del EventObf, SetObf, EntrantObf, GameObf, PhaseObf, TournamentObf, PersonalInformationObf,CharacterObf
	...
	def retrieve(self, tournamentID=None, tournamentTitle=None, eventName=None):
		...
		if tournamentID is None and tournamentTitle is None:
			...
			print(
				"Warning: Title retrival will retrieve the latest tournament entered with the title"
			)
		...
		set_ex = MatchSet.MatchSet(tournamentID=tourn_ex.tournamentID)
	...
	def store(self, bracket=None):
		...
		tourn_bracket = Tournament.Tournament(
			tournamentTitle=bracket["title"],
			tournamentID=bracket["tournamentID"],
			obfversion=bracket["version"],
			description=bracket["description"],
			SetGameResult=bracket["SetGameResult"],
		)
		...
		for eventx in event_list:
			# 			print(eventx)
			event_obf = eventx
			event_bracket = Event.Event(
				eventDate=event_obf["eventDate"],
				gameName=event_obf["gameName"],
				ruleset=event_obf["ruleset"],
				originURL=event_obf["originURL"],
				name=event_obf["name"],
				numberEntrants=event_obf["numberEntrants"],
			...
			if "phases" in bracket:
				...
				phase_bracket = Phase.Phase(
					phaseID=None, phaseStructure=None, name=event_bracket.name
				)
		for i in range(0, len(bracket["sets"])):
			...
			set_bracket = MatchSet.MatchSet(
				entrant1ID=set_bracket_parse["entrant1ID"],
				entrant2ID=set_bracket_parse["entrant2ID"],
				status=set_bracket_parse["status"],
			...
			for jx in game_bracket_set:
				...
				game_bracket = Game.Game(
					gameNumber=game_bracket_parse["gameNumber"],
		...
		for i in range(0, len(bracket["entrants"])):
			...
			entrant_Data = Entrant.Entrant(
				entrantID=entrant_parse["entrantTag"],
